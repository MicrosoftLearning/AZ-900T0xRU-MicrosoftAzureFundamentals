...
---
# 19 — использование калькулятора цен Azure
...
# Задача 1. Настройка калькулятора цен (10 мин)
...
4. Измените конфигурацию виртуальной машины по умолчанию.
    ...
    | Регион | Операционная система | Тип |
    |------|----------------|----|
    | Северная Европа | Windows | (Только ОС) |
    | | |

    | Уровень | Экземпляр |
    |----|--------|
    | Стандартный | A2: 2 ядра, 3,5 ГБ ОЗУ, временное хранилище на 135 ГБ |
    | | |
...
11. Добавьте имя для конфигурации шлюза приложений. В этом примере используется имя **App Gateway: Windows VM** (Пропускная способность: виртуальная машина Windows). Измените конфигурацию шлюза приложений по умолчанию, добавив следующие сведения.
    ...
    | Регион | Уровень | Размер |
    | ------ | ---- | ---- |
    | Северная Европа | Базовый | Маленький |
    | | |
    ...
    | Зона 1: Северная Америка, Европа |
    | ----------------------------- |
    | 50 ГБ |
    | | |

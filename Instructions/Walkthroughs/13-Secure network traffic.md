...
2. В колонке **Все службы** найдите и выберите элемент **Виртуальные машины**, а затем щелкните **+ Добавить**.
...
3. На вкладке **Основные** укажите следующие сведения (для остальных параметров оставьте значения по умолчанию):
    ...
    | Группа ресурсов | **myRGSecure** (создайте новую) |
...
1. В колонке **Все службы** найдите и выберите элемент **Группы безопасности сети**, а затем щелкните **+ Добавить**.
...
2. На вкладке **Основные** в колонке **Создание группы безопасности сети** укажите следующие параметры.
    ...
    | -- | -- |
    | Подписка | **Выберите свою подписку.** |
    | Группа ресурсов | **myRGSecure** |
    | Имя | **myNSGSecure** |
    | Регион | **(США) Восточная часть США**  |
    | | |

3. Нажмите кнопку **Просмотр и создание**, а после проверки нажмите затем кнопку **Создать**.
...
3. Попытайтесь подключиться к виртуальной машине по протоколу RDP. По умолчанию группа безопасности сети не разрешает использовать RDP. Закройте окно ошибки. 
...
6. Подождите, пока правило будет подготовлено, а затем повторите попытку подключения к виртуальной машине по протоколу RDP. На этот раз все должно получиться. Помните, что используются имя пользователя **azureuser** и пароль **Pa$$w0rd1234**.
...
4. На портале Azure вернитесь к колонке виртуальной машины **SimpleWinVM**. 
